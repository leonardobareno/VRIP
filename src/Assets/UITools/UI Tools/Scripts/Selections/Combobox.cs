...
public class Combobox : MonoBehaviour
{
    ...
    void OnValidate()
    {
        if (bDebug) Debug.Log("***** OnValidate()");
    ...
    void UpdateDropdown()
    {
        if (bDebug) Debug.Log("***** UpdateDropdown()");
        ...
        try
        {
            ...
            //UpdateDropdown();
            if (bDebug) Debug.Log("***** DROPDOWN LIST NULL, ABORTA");
            return;
        }

        if (dropdownDisplay != null)
        {
            if (bDebug) Debug.Log("***** dropdownDisplay != null");

            //SUSTITUIDO POR CanvasVRModifier
            //Cambiar GraphicRaycaster por OVRRaycaster
            /*if (!bVRAdj_exec) {
                GraphicRaycaster gr = dropdown.transform.Find("Dropdown List").GetComponent<GraphicRaycaster>();
                OVRRaycaster OVRrc = null;
                if (gr != null) {
                    gr.enabled = false;
                    OVRrc = dropdown.transform.Find("Dropdown List").gameObject.AddComponent<OVRRaycaster>();
                    OVRrc.pointer = OVRGazePointer.instance.gameObject;
